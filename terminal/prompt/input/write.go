...
func WriteAnswer(t interface{}, name string, v interface{}) (err error) {
	...
	case reflect.Struct:
		...
		// TODO: This type of string compareison is really bad, at least start with
		// a len check so we can int check before needing a string check
	...
	case reflect.Map:
		...
		if mapType.Key().Kind() != reflect.String {
			return fmt.Errorf("answer maps key must be of type string")
		}
...
func (self errFieldNotMatch) Error() string {
	return fmt.Sprintf("could not find field matching %v", self.questionName)
}
...
func (err errFieldNotMatch) Is(target error) bool { // implements the dynamic errors.Is interface.
	if target != nil {
		if name, ok := IsFieldNotMatch(target); ok {
			...
			return len(name) == 0 || len(err.questionName) == 0 || name == err.questionName
		}
	}
