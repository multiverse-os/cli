module github.com/multiverse-os/cli

go 1.19

require (
	github.com/multiverse-os/ansi v0.1.0
	github.com/multiverse-os/banner v0.1.0
	github.com/multiverse-os/cli/terminal/loading v0.0.0-00010101000000-000000000000
	github.com/multiverse-os/color v0.1.0
)

require golang.org/x/text v0.13.0 // indirect

replace github.com/multiverse-os/loading => github.com/multiverse-os/cli/terminal/loading v0.1.0

replace github.com/multiverse-os/cli/terminal/loading => ./terminal/loading
